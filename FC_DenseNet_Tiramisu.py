...
def preact_conv(inputs, n_filters, filter_size=[3, 3], dropout_p=0.2, is_bottneck=False):
    ...
    """
    if is_bottneck:
      preact = tf.nn.relu(slim.batch_norm(inputs, data_format='NCHW'))
      inputs = slim.conv2d(preact, 4*n_filters, [1, 1])
    preact = slim.batch_norm(inputs, data_format='NCHW')
    conv = slim.conv2d(preact, n_filters, filter_size)
    if dropout_p != 0.0:
...
def TransitionDown(inputs, n_filters, dropout_p=0.2, compression_rate=1, scope=None):
  ...
  with tf.name_scope(scope) as sc:
    ...
    l = slim.pool(l, [2, 2], stride=[2, 2], pooling_type='MAX', data_format='NCHW')
    return l
...
def build_fc_densenet(inputs, preset_model='FC-DenseNet56', num_classes=12, n_filters_first_conv=48, n_pool=5, growth_rate=12, n_layers_per_block=4, dropout_p=0.2, is_bottneck=False, compression_rate=1, data_format='NCHW', scope=None):
    """
    ...
    with tf.variable_scope(scope, preset_model, [inputs]) as sc:
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            #      Softmax      #
            #####################
            network = slim.conv2d(stack, num_classes, [1, 1], scope='logits')
            if num_classes>1:
                prob = tf.nn.softmax(network, 1) #defatult is -1 for NHWC, but here we use NCHW
            else:
                prob = tf.nn.sigmoid(network)
            variables = tf.contrib.framework.get_variables(sc) #vs
            return network, prob, variables