...
gt = tf.placeholder(tf.float32,shape=[None,6,None,None],name='gt_map')
G_logist, G, G_var = Segmentor(z, preset_model='FC-DenseNet56', num_classes=6, data_format=data_format, reuse=False)
# print(G_var)
...
# with tf.Session() as sess_init:
   # variables_to_restore = slim.get_variables_to_restore(exclude=["FC-DenseNet56/logits/biases","FC-DenseNet56/logits/weights"])
...
##############
d_out, GG, xx, D_var = Discriminator_small(z, gt, G, z_num, repeat_num, conv_hidden_num, data_format)
...
d_loss_real = (0.01+tf.reduce_sum(gt_1d*tf.abs(AE_x - xx)))/(tf.reduce_sum(gt_1d)+0.01)
d_loss_fake = (0.01+tf.reduce_sum(gt_1d*tf.abs(AE_G - GG)))/(tf.reduce_sum(gt_1d)+0.01)
...
# w1 = tf.get_variable("FC-DenseNet56/first_conv/weights",reuse=True)
# print(w1.shape,type(w1))
summary_op = tf.summary.merge([
    ...
    # tf.summary.histogram("lx/gt_hist", ),
])
...
for step in trange(start_step, max_step):
    ...
    if step % 2000 == 2000-1:
        ...
        for id in val_indices:
            input_image = np.expand_dims(cv2.cvtColor(cv2.imread(val_input_names[id],-1), cv2.COLOR_BGR2RGB),0)/255
            gt_map = np.expand_dims(cv2.imread(val_output_names[id],-1),2)
            #gt_map /= np.max(gt_map)
            input_image = np.transpose(input_image, [0, 3, 1, 2])
            ...
            class_accuracies = utils_seg.compute_class_accuracies(output_image, gt_map, 6)
